...
Significant planning discussions, architectural decisions, and plan proposals are captured in `PLANNING.md`. Latest entries at top; each entry is date-stamped. When a conversation produces a meaningful design decision or options analysis, log it there before closing context.
...
### NextText Hierarchy (from PLANNING.md)

| # | Step | Status |
|---|------|--------|
| 1 | Node factories (`makeWord`, `makeLine`, `makePassage`, `makeDocument`) | ✅ Done |
| 2 | `transferNodeBehaviours()` helper | ✅ Done |
| 3 | `layout()` Stage B — build hierarchy tree, set back-pointers | ✅ Done |
| 4 | AgentEngine Phase 5A/5B split + Phase 6A node dynamics + node-as-agent unit behaviours (`cascade: false`) | ✅ Done |
| 4b | Mixed-level selection → LCA | ⬜ Next |
| 4c | Save format — emit/consume full hierarchy JSON (v2: nodes[] + glyphs[] + params) | ✅ Done |
| 5 | Selection model — `selectedNode`, `selectNode()`, `leafGlyphsOf()`, `nodeTypeLabel()` | ✅ Done |
| 6 | Hierarchy-aware behaviour panel — level indicator, cascade toggle (Unit/Cascade), geo/agent hide for non-glyph | ✅ Done |
| 7 | `addBehaviour` / `removeBehaviour` / `toggleCascadeAtIndex` target `selectedNode` | ✅ Done |
| 8 | Undo/Redo — snapshot node behaviours (`nodeBehaviours` plain object keyed by node id) | ✅ Done |

### Phase 2 — opentype.js Bezier Rendering (from PLANNING.md)

| # | Feature | Status |
|---|---------|--------|
| 1 | opentype.js CDN script in `<head>` | ✅ Done |
| 2 | `otFont` global state | ✅ Done |
| 3 | `buildCharPath()` + `_makeBezierData()` helpers | ✅ Done |
| 4 | `makeGlyph` defaults to `outline2d`; bezier is opt-in per glyph | ✅ Done |
| 5 | Post-layout stale-path rebuild (bezier glyphs only, on fontPt change) | ✅ Done |
| 6 | `loadOTFontFile()` async loader; label update | ✅ Done |
| 7 | `outline2d-bezier` render branch (compose + fast path, full highlight logic) | ✅ Done |
| 8 | Panel: Font file loader (Type section), Bezier button (Geometry panel), Show handles checkbox | ✅ Done |
| + | Bezier handle debug overlay (`drawBezierHandles`, `_dbgAnchor`, `_dbgHandle`) | ✅ Done |

---

## Deferred: particle geometry + node-transform composition

The `drawParticleGeometry` function handles its own positioning and does not use `ctx` transforms. Glyphs with particle geometry therefore do not yet inherit ancestor node transforms (`cascade: false` motion on a Word/Line/Passage has no visible effect on their particles). This requires baking the node's displacement into the particle system's origin. Deferred to a later step.

**Workaround:** Use `cascade: true` for behaviours on nodes that contain particle-geometry glyphs — that cascades the force directly onto the particle glyph's own dynamics, which works today.
...
Glyphkicker/
  ...
  CLAUDE.md           — this file
  PLANNING.md         — planning log (latest entries at top)
